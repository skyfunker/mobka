...
//    $('#workingDate').text(moment().format('DD/MM/YYYY'));
    $("#grid-data").bootgrid({
        ...
        caseSensitive: true,
        ajax: true,
        url: 'http://cwport.com/mobka/data/cashops.json',
        ajaxSettings: {
            method: "GET",
            cache: true
        },
        ...
        formatters: {
            "flags": function (column, row) {
                var html = '<span class="f-20 c-gray" title="Включено"><i class="zmdi ' 
                        + (row.inCount == '1' ? 'zmdi-check' : 'zmdi-close') + ' zmdi-hc-fw" data-row-id="'
                        + row.cashopId + '"></i></span>';
                html += '<span class="f-20 c-gray" title=""><i class="zmdi ' 
                        + (row.opType == '-1' ? 'zmdi-minus' : 'zmdi-plus') + ' zmdi-hc-fw" data-row-id="'
                        + row.cashopId + '"></i></span>';
                return html;
            },
